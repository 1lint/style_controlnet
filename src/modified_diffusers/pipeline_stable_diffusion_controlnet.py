...
class StableDiffusionControlNetPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if controlnet_prompt is not None:
            ...
            controlnet_prompt_embeds = torch.cat(
                [negative_prompt_embeds, controlnet_prompt_embeds]
            )
            ...
            for i, t in enumerate(timesteps):
                ...
                for res_sample in down_block_res_samples + (mid_block_res_sample,):
                    if controlnet_mode is None:
                        ...
                        conditioning = torch.linspace(
                            conditioning_start,
                            conditioning_end,
                            res_sample.shape[cond_index],
                        ).to(res_sample)
                ...
                #for res_sample in down_block_res_samples + (mid_block_res_sample,):
                #    res_sample *= controlnet_conditioning_scale
