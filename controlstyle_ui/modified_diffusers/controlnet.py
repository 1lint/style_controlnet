...
from diffusers.pipelines.stable_diffusion.convert_from_ckpt import convert_ldm_unet_checkpoint, create_unet_diffusers_config
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        elif controlnet_conditioning_embedding_type == "null":
            pass
        else:
            raise ValueError(
                f"unknown `controlnet_conditioning_embedding_type`: {controlnet_conditioning_embedding_type}. Options are 'default', 'null'"
            )
    ...
    def load_weights_from_diffusers_unet(self, *args, **kwargs):

        if 'subfolder' not in kwargs:
            kwargs['subfolder'] = 'unet'

        unet = UNet2DConditionModel.from_pretrained(*args, **kwargs)

        # expect missing keys warning for zero conv modules (controlnet_down_blocks and controlnet_mid_block)
        self.load_state_dict(unet.state_dict(), strict=False)

    # copied from/based on diffusers convert_from_ckpt.py
    def load_weights_from_sd_ckpt(self, ckpt_path):
        ...
        if key_name in checkpoint and checkpoint[key_name].shape[-1] == 1024:
            config_file_name = 'v2-inference-v.yaml'
            upcast_attention = True
            image_size = 768
        else:
            config_file_name = 'v1-inference.yaml'
            upcast_attention = False
            image_size = 512

        original_config_file = hf_hub_download(
            'lint/stable_diffusion_configs',
        ...
        # expect missing keys warning for zero conv modules (controlnet_down_blocks and controlnet_mid_block)
        self.load_state_dict(converted_unet_checkpoint, strict=False)
